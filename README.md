...
OBS 排队挂件适配 360×400；在“连接与规则”选择自动翻页（人数按高度调整）或自动滚动。当前观众始终固定，全部等待观众轮流显示。

控制台显示版本、运行目录和地址，并检查常用端口上的其他副本。提供主播专用弹幕诊断预览、20 秒内撤销最近一次移除/叫号推进，以及翻页间隔、滚动速度、字号和金额显示选项。普通入队时尽量保持名单展示进度。
