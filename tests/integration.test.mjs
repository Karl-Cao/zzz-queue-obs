...
test('服务集成：局域网配对、权限、模拟操作、礼物导入与持久化', { timeout: 15000 }, async () => {
  ...
  try {
    ...
    assert.equal(local.settings.roomId, '');assert.equal(local.runtime.version,'1.9.0');assert.ok(local.runtime.directory);assert.equal((await (await fetch(base+'/api/state')).json()).runtime,undefined);
